...
public class TabView extends RelativeLayout {
    ...
    //消息提示显示
    private TextView tab_red_circle;
    //透明度
...
//        mTabSelectIcon = typedArray.getDrawable(R.styleable.TabView_tab_iconSelect);
    ...
    public void setmTabIcon(String mTabIcon) {
        this.mTabIcon = mTabIcon;
    }

    public String getmTabSelectIcon() {
        return mTabSelectIcon;
    }

    public void setmTabSelectIcon(String mTabSelectIcon) {
        this.mTabSelectIcon = mTabSelectIcon;
    }
    ...
    private void setTabIconData() {
        KLog.d(getmTabIcon());
        if (IsNullUtil.getInstance().isEmpty(getmTabIcon())) {
            GlideLoadUtils.getInstance().glideLoad(mContext, DefaultUnSelectImage(), mTabIM, DefaultUnSelectImage());
        } else {
            GlideLoadUtils.getInstance().glideLoad(mContext, getmTabIcon(), mTabIM, DefaultUnSelectImage());
        }
        if (isChecked) {
    ...
    private void setTabIconSelectData() {
        if (IsNullUtil.getInstance().isEmpty(getmTabSelectIcon())) {
            GlideLoadUtils.getInstance().glideLoad(mContext, DefaultSelectImage(), mTabSelectIM, DefaultSelectImage());
        } else {
            GlideLoadUtils.getInstance().glideLoad(mContext, getmTabSelectIcon(), mTabSelectIM, DefaultSelectImage());
        }
        if (isChecked) {
    ...
    /**
     * description: 根据tab名称获取默认选中图片
     * author: Andy
     * date: 2020/4/15 0015 11:04
     */
    private int DefaultSelectImage() {
        if (IsNullUtil.getInstance().isEmpty(getmTabName())) return R.mipmap.default_icon_image;
        int imagePath = 0;
        switch (getmTabName()) {
            case "首页":
                imagePath = R.mipmap.tab_home_select_icon;
                break;
            case "探索":
                imagePath = R.mipmap.tab_search_select_icon;
                break;
            case "消息":
                imagePath = R.mipmap.tab_message_select_icon;
                break;
            case "我的":
                imagePath = R.mipmap.tab_mine_select_icon;
                break;
        }
        return imagePath;
    }
    ...
    /**
     * description: 根据tab名称获取默认未选中图片
     * author: Andy
     * date: 2020/4/15 0015 11:07
     */
    private int DefaultUnSelectImage() {
        if (IsNullUtil.getInstance().isEmpty(getmTabName())) return R.mipmap.default_icon_image;
        int imagePath = 0;
        switch (getmTabName()) {
            case "首页":
                imagePath = R.mipmap.tab_home_icon;
                break;
            case "探索":
                imagePath = R.mipmap.tab_search_icon;
                break;
            case "消息":
                imagePath = R.mipmap.tab_message_icon;
                break;
            case "我的":
                imagePath = R.mipmap.tab_mine_icon;
                break;
        }
        return imagePath;
    }
     ...
     * 设置默认状态下的name的样式
     */
    public void setNameData(String mTabName) {
    ...
    public int getmTabSelectColor() {
        return mTabSelectColor;
    }

    public void setmTabSelectColor(int mTabSelectColor) {
        this.mTabSelectColor = mTabSelectColor;
    }

    /**
     * 设置选中状态下的name的样式
     */
    public void setNameSelectData(String mTabName) {
        mTabSelectTv.setText(mTabName);
        mTabSelectTv.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTabNameSize);
        mTabSelectTv.setTextColor(getmTabSelectColor());
        if (isChecked) {
